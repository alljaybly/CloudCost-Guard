...
function App() {
  ...
  const handleAnalyze = useCallback(async (data: string) => {
    ...
    setError(null);
    try {
      const result = await analyzeBillingData(data);
      setAnalysisResult(result);
    } catch (err) {
      setError('Failed to analyze billing data. Please try again.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans">
      ...
      <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        <CostAnalysis onAnalyze={handleAnalyze} initialData={SAMPLE_BILLING_DATA} isLoading={isLoading} />
